...
Motores carro(6, 7, 8, 9, 10, 11); // ENA,IN1,IN2,IN3,IN4,ENB in that order
LineSensor rightLine(A6);
LineSensor leftLine(A7);

#define MAX_DISTANCE 35 // Maximum distance we want to ping for (in centimeters). Maximum sensor distance is rated at 400-500cm.
///NewPing sonar(5, 4, MAX_DISTANCE); //Trigger, ECHO, MAX_DISTANCE // NewPing setup of pins and maximum distance.
int distance;
...
#ifdef SENSOR_TESTER
void loop() {
  distance = 30; //sonar.ping_cm(); // Send ping, get distance in cm and print result (0 = outside set distance range)
