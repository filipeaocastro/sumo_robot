...
long time_serial = 0;
const int intervalo_serial = 500;
...
void setup()
{
    ...
    pinMode(TSSOP_DIR, INPUT_PULLUP);
    pinMode(TSSOP_ESQ, INPUT_PULLUP);
...
void loop()
{
    ...
    else
    {
        ...
        if(!inimigo_esquerda && inimigo_centro && !inimigo_direita)
        {
            ...
            //else if(distancia >= 10)
            //    praFrente(70, 'T');
            else if( (distancia >= 0) && (distancia <=15) )
                praFrente(100, 'T');
        }
    }

    if(time_now - time_serial >= intervalo_serial)
    {
        Serial.println(inimigo_esquerda);
        Serial.println(distancia);
        Serial.println(inimigo_direita);
        Serial.println("\n");
